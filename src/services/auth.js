...
import { MDBModal, MDBModalHeader, MDBModalBody, MDBModalFooter, MDBBtn } from 'mdb-react-ui-kit'; // Uvoz komponenti modala
    ...
    const login = async (email, sifra) => {
        try {
            ...
            if (response.status === 200) {
                ...
                navigate('/dashboard'); // Preusmeravanje na dashboard stranicu
                return loggedInUser; 
            } else {
                setShowModal(true); // Prikaz modala za neuspešnu prijavu
                setModalMessage('Neuspešna prijava. Proverite vaše korisničke podatke i pokušajte ponovo.');
                console.error('Login failed:', response.data.message);
                return null;
            }
        } catch (error) {
            setModalMessage('Došlo je do greške prilikom prijave. Molimo pokušajte ponovo kasnije.');
